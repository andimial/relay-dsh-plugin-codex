...
import { LlmAdapter, LlmError, MessageId, ToolCallId, createMessage, freezeMessage } from "@deepseek-ai/dsh-llm";
...
function codexDshToolSurface(dshTools = [], builtins = CODEX_APP_DYNAMIC_TOOLS) {
	const namesByAlias = /* @__PURE__ */ new Map();
...
function registerDshToolAlias(namesByAlias, name) {
	if (typeof name !== "string" || !name) throw new Error("DSH tool name must be a non-empty string");
	const alias = codexSafeDshToolName(name);
	const existing = namesByAlias.get(alias);
	if (existing === name) throw new Error(`Duplicate DSH tool name: ${name}`);
	if (existing !== void 0) throw new Error(`DSH tool alias collision between ${existing} and ${name}: ${alias}`);
...
function isCodexSafeDshToolName(name) {
	return name.length <= CODEX_DYNAMIC_TOOL_NAME_MAX_LENGTH && CODEX_DYNAMIC_TOOL_NAME_PATTERN.test(name) && name !== "mcp" && !name.startsWith("mcp__");
}
...
var CodexDshAdapter = class extends LlmAdapter {
	constructor({ runtime, ready, linkStore = null, attachments = null, logger = console, dynamicTools = CODEX_APP_DYNAMIC_TOOLS, executionGuidance = true, executionMode = "enhanced" }) {
		...
		this.dshToolNamesByAlias = /* @__PURE__ */ new Map();
		this.forwardedSkillCatalogs = /* @__PURE__ */ new Map();
	...
	replaceImportedSession(oldSessionId, newSessionId) {
		...
		if (hasForwardedSkillCatalog) this.forwardedSkillCatalogs.set(newKey, forwardedSkillCatalog);
	...
	dshToolName(sessionId, alias) {
		const key = String(sessionId);
		return this.dshToolNamesByAlias.get(key)?.get(alias) ?? (this.dshToolNames.get(key)?.has(alias) === true ? alias : null);
	}
	...
	async *stream(options) {
		...
		const projectedInput = await latestUserInput(options.messages, this.forwardedSkillCatalogs.get(sessionId) ?? null, this.attachments, options.signal);
		if (!projectedInput) throw new Error("Relay Codex adapter received no user text or image input");
		const input = {
			text: projectedInput.text,
			localImages: projectedInput.localImages
		};
		...
		try {
			...
			if (projectedInput.skillCatalog !== null) this.forwardedSkillCatalogs.set(sessionId, projectedInput.skillCatalog);
			turnId = started.id;
...
async function latestUserInput(messages, forwardedSkillCatalog, attachments, signal) {
	...
	for (let index = 0; index < messages.length; index += 1) {
		...
		if (index > inputIndex && isSkillInvocation(message.source) && text) skillContext.push({
			index,
			text
		});
	}
	if (skillCatalog && skillCatalog !== forwardedSkillCatalog) {
		const catalogIndex = messages.findLastIndex((message) => message?.role === "user" && isSkillCatalog(message.source));
		skillContext.push({
			index: catalogIndex,
			text: skillCatalog
		});
...
function isSkillInvocation(source) {
	return source?.kind === "skill-invocation" && source.form === "instructions" && typeof source.name === "string" && source.name.length > 0;
}
