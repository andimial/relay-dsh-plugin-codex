...
function latestUserInput(messages) {
	...
	const skillInstructions = messages.slice(inputIndex + 1).filter((message) => message?.role === "user" && isSkillInvocation(message.source)).map((message) => (message.content ?? []).filter((block) => block.type === "text").map((block) => block.text).join("\n").trim()).filter(Boolean);
	return {
