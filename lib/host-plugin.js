...
var CodexDshAdapter = class extends LlmAdapter {
	...
	async createOrResumeThread(sessionId, dynamicTools) {
		...
		if (linked) try {
			...
			if (this.bindingModes.get(sessionId) === "imported") throw importedResumeError(linked, error);
...
function importedResumeError(threadId, cause) {
	if (isActiveWriterError(cause)) {
		const error = new Error(`Codex thread ${threadId} is still owned by another Codex App Server. Switching Sessions may not release this process-level writer. Fully quit or restart the owning Codex app, CLI, or App Server process, then retry this message in DSH. DSH kept the original thread binding and did not create a replacement.`, { cause });
