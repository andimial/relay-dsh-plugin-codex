		...
		*/
		...
		const codexProcessDefinition = {
			...
			match: (event) => {
				...
				if (event.type === "turn/end" || event.type === "assistant/chunk" || event.type === "assistant/message" || event.type === "tool/result" || event.type === "tool/call") {
		...
		function GroupedCodexToolActivityView(props) {
			const process = props.useSession((snapshot) => {
				const location = snapshot.chat.nodes.get((0, _deepseek_ai_dsh_client_runtime_client.conversationContextKey)("tool-call", props.callId))?.location;
		...
		function CodexProcessView({ node, sessionId, renderMessageImages, useTurnData, useSession, openFile, fileMentions }) {
			const legacyBoundary = useSession((snapshot) => [...snapshot.chat.nodes.values()].some((candidate) => candidate.kind === "relay-codex-activity" && candidate.anchorSeq >= (node.data.firstVisibleSeq ?? Infinity) && candidate.anchorSeq <= (node.data.lastSeq ?? -1)));
