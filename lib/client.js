	...
	factory: (require) => {
		...
		function installModelSelection(ctx, preset, provider, otherProvider, { retryDelaysMs = DEFAULT_RETRY_DELAYS_MS } = {}) {
			...
			const currentTarget = (id, target) => {
				...
				return !stopped && list.current === id && latest?.blank === true && desired.get(id)?.generation === target.generation && latest.projectionValues?.agentPreset === target.selectedPreset;
			};
		...
		*/
		...
		function CodexProcessView({ node, sessionId, renderMessageImages, useTurnData, useChat, openFile, fileMentions, turnProcess }) {
			const legacyBoundary = useChat((snapshot) => [...snapshot.nodes.values()].some((candidate) => candidate.kind === "relay-codex-activity" && candidate.anchorSeq >= (node.data.firstVisibleSeq ?? Infinity) && candidate.anchorSeq <= (node.data.lastSeq ?? -1)));
		...
		async function refreshImportedWorkspace(sessions) {
		...
		function observeSessionOpen(currentSessionSource, syncSession, onError = console.warn, readFallbackSessionId = readPersistedDshCurrentSessionId) {
		...
		function apply(ctx) {
			...
			installProcessPresentation(ctx, applyAdvancedDebug(ctx));
		...
		function applySessionOpenSync(ctx) {
			ctx.effect(() => observeSessionOpen(ctx.sessions.list, (sessionId, isLatestSelection) => syncOpenedCodexSessionAndRefresh(sessionId, () => ctx.sessions.refresh(), fetch, (rebuiltSessionId) => ctx.sessions.open(rebuiltSessionId), void 0, isLatestSelection), (error) => console.warn("Codex open-time history sync failed:", error)), "relay-codex: open-time history sync");
		}
